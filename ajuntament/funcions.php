...
*****************************************************/
function mostrar_informacio_ambit($ambit){
	include ("../conn_bd.php");
	...
	$res = $mysqli->query("SELECT * FROM item WHERE ambit = ".$ambit." AND actiu = 1 ORDER BY id ASC");
...
**********************************************************************/
function numero_items_publicats($ambit){
	...
	if($ambit == 0){//publicats
		$res = $mysqli->query("SELECT count(*) as total_publicats FROM item WHERE publicat = 1 AND actiu = 1");
	}else if($ambit == 99){//no publicats
		$res = $mysqli->query("SELECT count(*) as total_publicats FROM item WHERE publicat = 0 AND actiu = 1");
	}else{
		$res = $mysqli->query("SELECT count(*) as total_publicats FROM item WHERE ambit = ".$ambit." AND publicat = 1 AND actiu = 1");
	}
...
********************************************************************/
function numero_items_totals($ambit){
	...
	if($ambit == 0){
		$res = $mysqli->query("SELECT count(*) as total_items FROM item WHERE actiu = 1");
	}else{
		$res = $mysqli->query("SELECT count(*) as total_items FROM item WHERE ambit = ".$ambit." AND actiu = 1");
	}
...
*********************************************************************************/
function numero_generacio_contingut($generacio){
	...
	$res = $mysqli->query("SELECT count(*) as total_items FROM item WHERE generacio_contingut = '".$generacio."' AND actiu = 1");
...
/*****************************************
Mostra les observacions generals d'un ens
*****************************************/
function observacions($id){
	...
	$res = $mysqli->query("SELECT * FROM observacions_generals WHERE id=".$id."");
	
	$row = $res->fetch_assoc();
	
	?>
	<form role="form" action="actualitzar_observacions.php?id=<?php echo $row["id"]; ?>" method="post" enctype="multipart/form-data" name="form" id="form">
		<div class="list-group">
			<div class="form-group">
				<textarea class="form-control" rows="3" name="text_area"><?php echo $row["contingut"]; ?></textarea>
			</div>
			 <button type="submit" class="btn btn-default">Actualitzar</button>
		</div>
	</form>
	<?php
...
***********************************************************************************************/
function informacio_detallat($ID_Item, $ambit){
	include ("../conn_bd.php");
	$mysqli = new mysqli($server, $user, $pwd, $bd);
	if ($mysqli->connect_errno) {
		echo "Failed to connect to MySQL: (" . $mysqli->connect_errno . ") " . $mysqli->connect_error;
	...
	$res = $mysqli->query("SELECT * FROM item WHERE ambit = ".$ambit." AND id = ".$ID_Item."");
	$row = $res->fetch_assoc();
