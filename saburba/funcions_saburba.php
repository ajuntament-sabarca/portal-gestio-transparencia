...
*****************************************************/
function mostrar_informacio_ambit_saburba($ambit){
	include ("../conn_bd.php");
	...
	$res = $mysqli->query("SELECT * FROM item_saburba WHERE ambit = ".$ambit." AND actiu = 1 ORDER BY id ASC");
...
**********************************************************************/
function numero_items_publicats_saburba($ambit){
	...
	if($ambit == 0){//publicats
		$res = $mysqli->query("SELECT count(*) as total_publicats FROM item_saburba WHERE publicat = 1 AND actiu = 1 ");
	}else if($ambit == 99){//no publicats
		$res = $mysqli->query("SELECT count(*) as total_publicats FROM item_saburba WHERE publicat = 0 AND actiu = 1 ");
	}else{
		$res = $mysqli->query("SELECT count(*) as total_publicats FROM item_saburba WHERE ambit = ".$ambit." AND publicat = 1 AND actiu = 1 ");
	}
...
********************************************************************/
function numero_items_totals_saburba($ambit){
	...
	if($ambit == 0){
		$res = $mysqli->query("SELECT count(*) as total_item_saburbas FROM item_saburba WHERE actiu = 1 ");
	}else{
		$res = $mysqli->query("SELECT count(*) as total_item_saburbas FROM item_saburba WHERE ambit = ".$ambit." AND actiu = 1 ");
	}
	...
	return $row['total_item_saburbas'];
}
/*********************************************************************************
Calcula el número d'ítems automàtics o manuals d'un àmbit per mostrar-lo al resum
*********************************************************************************/
function numero_generacio_contingut_saburba($generacio){
	...
	$res = $mysqli->query("SELECT count(*) as total_item_saburbas FROM item_saburba WHERE generacio_contingut = '".$generacio."' AND actiu = 1 ");
...
***********************************************************************************************/
function informacio_detallat_saburba($ID_Item, $ambit){
	include ("../conn_bd.php");
	$mysqli = new mysqli($server, $user, $pwd, $bd);
	if ($mysqli->connect_errno) {
		echo "Failed to connect to MySQL: (" . $mysqli->connect_errno . ") " . $mysqli->connect_error;
	...
	$res = $mysqli->query("SELECT * FROM item_saburba WHERE ambit = ".$ambit." AND id = ".$ID_Item."");
	$row = $res->fetch_assoc();
	...
	<form role="form" action="actualitzar_dades.php?i=<?php echo $id_xifrat; ?>&a=<?php echo $row['ambit']; ?>" method="post" enctype="multipart/form-data" name="form1" id="form1">
		...
		<td><div class="form-group">
			...
			<textarea class="form-control" maxlength="656" rows="10" name="descripcio" onKeyDown="cuenta()" onKeyUp="cuenta()"><?php echo $row['descripcio']; ?></textarea>
