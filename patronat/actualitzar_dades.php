...
$query = "UPDATE item_patronat SET nom = '".$nom."', ambit = ".$ambit.", prioritat = '".$prioritat."', generacio_contingut = '".$g_contingut."', ltcat = '".$ltcat."', enllas_directe = '".$enllas_directe."', persona_contacte = '".$p_contacte."', observacions = '".$observacions."', estat = ".$estat.", publicat = ".$publicat.", descripcio = '".$descripcio."', darrera_actualitzacio= '".$darrera_actu."', data_revisio = '".$data_actualitza."' WHERE id = ".$id_item." AND ambit =".$ambit."";
...
if ($mysqli->connect_errno) {
	...
	if($res = $mysqli->query($query)){//realitzar la query
		...
		$actualitzacio = actualitza_historial_actualitzacions($mysqli, $data_actualitza, $_SESSION["id_usuari"], 2, $ambit);
		...
		echo "No s'ha pogut canviar les dades de l'item";
	}
